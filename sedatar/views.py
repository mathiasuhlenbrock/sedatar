...
def post(request):
    database = get_object_or_404(Database, name = database_page_name.replace("_", " "))

    return render_to_response('sedatar/database.html', {'database': database})
...
def list_of_databases(request):
    ...
    try:
        list_of_databases = paginator.page(page)
    except (EmptyPage, InvalidPage):
        list_of_databases = paginator.page(paginator.num_pages)

    return render_to_response('sedatar/list_of_databases.html', {'list_of_databases': list_of_databases})
...
def list_of_posts(request):
    ...
    return render_to_response('sedatar/list_of_posts.html', {'list_of_posts': list_of_posts})

def post(request, post_date):
    post = get_object_or_404(Post, date = post_date)

    return render_to_response('sedatar/post.html', {'post': post})
