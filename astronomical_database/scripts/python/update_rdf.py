...
with open('astronomical_database/data/csv/planets/planets.csv') as csvfile:
    ...
    for row in reader:
        ...
        f = open("astronomical_database/data/rdf/individuals/" + row['pl_hostname'].replace(' ', '_') + "_" + row['pl_letter'] + ".rdf", 'w')

        f.write("<rdf:Description rdf:about=\"urn://sedatar.org/astronomical_database/individuals/" + row['pl_hostname'].replace(' ', '_') + "_" + row['pl_letter'] + "\">\n")
        f.write("  <rdf:type rdf:resource=\"http://www.w3.org/2000/01/rdf-schema#Class\" />\n")
        ...
        f.write("  <rdfs:subClassOf rdf:resource=\"urn://sedatar.org/astronomical_database/astronomy/Exoplanet\" />\n")
